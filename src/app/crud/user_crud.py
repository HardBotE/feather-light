...
from ..requests.user_model import RegisterUserModel
...
def register(credentials: RegisterUserModel):
    with Session(engine) as session:
        with session.begin():
            ...
            session.add(
                UsersTable(email=str(credentials.email), password=hashed_password)
            )
...
def does_user_exist(email):
    with Session(engine) as session:
        with session.begin():
            is_user_registered = (
                session.query(UsersTable).filter(UsersTable.email == email).first()
            )
